...
def test_args_method(parser):
    with pytest.raises(SystemExit):
        parse_args(
            ['--method', *default_metric_args, *default_inputs_args])

    parse_args(
        ['--method=CNH', *default_metric_args, *default_inputs_args])
    parse_args(
...
def test_args_probability(parser):
    ...
    parse_args(
        ['--probability=0', *default_metric_args, *default_inputs_args])
    parse_args(
        ['--probability=1', *default_metric_args, *default_inputs_args])
...
def test_args_confidence(parser):
    ...
    parse_args(
        ['--confidence=0', *default_metric_args, *default_inputs_args])
    parse_args(
        ['--confidence=1', *default_metric_args, *default_inputs_args])
